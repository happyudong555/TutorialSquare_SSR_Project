...
export default class Top_highlights_course_teacher extends React.Component {
    static getInitialProps() {
        return {course:course}
    }
    render(){
        let courseItem = course.slice(0,4).filter(course => course.category === "gat pat");
        return (
            <div className="container">
                <h3 className="HeadingTitle">top highlights</h3>
                <div className="row">
                    {
                        courseItem.map( (courseItem,i) => (
                        <div key={i} className="col-sm-6 col-md-3 card courseBox">
                            <div className="courseThumbnail">
                                <a href={`/teacher_content?id=${courseItem.id}`}>
                                ...
                                <span className="teacherName">happyudong</span>
                            </div>
                        </div>
                    ))
                    }
